...
// Preview the first note after starting the app :
function notePreviewLoaded(notes) {
...
function createNoteHTML(notes) {
  ...
  notes.forEach((note) => {
    ...
    <div class="notes__small-updated">${new Date(
      note.createdDate
    ).toLocaleString("en-US", { timeStyle: "short", dateStyle: "full" })}</div>
  </div>
      `;
  });
...
function createCurrentNote(currentNoteIndex) {
  const titleInput = document.querySelector(".notes__title");
  const bodyInput = document.querySelector(".notes__body");
  ...
  noteEdit(id, currentNote);
}

function noteEdit(id, note) {
  const preview = document.querySelector(".notes__preview");
...
*/
function setNoteStyle(removedNoteIndex) {
  if (removedNoteIndex == 0) {
    ...
    const currentNoteBody = currentNoteHTML.querySelector(".notes__small-body");
